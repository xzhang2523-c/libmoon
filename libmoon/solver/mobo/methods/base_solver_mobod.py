...
class MOBOD(object):
    ...
    def solve(self):
        ...
        if self.n_obj in (2, 3):
            self.ref_vecs = torch.from_numpy(
                get_reference_directions("uniform", self.n_obj, n_partitions=params_H[self.n_obj-2])
            )
    ...
    def _moead_GR(self, ref_vecs, pref_incs):
        ...
        maxIter = 50
        pop_size = int(self.ref_vecs.shape[0]) # pop_size
        T = int(np.ceil(0.1 * pop_size))  # size of neighbourhood: 0.1*N
        ...
        for gen in range(maxIter - 1):
            for i in range(pop_size):
                if torch.rand(1) < 0.8:  # delta
                    P = B[i, torch.randperm(B.shape[1])]
                else:
                    P = torch.randperm(pop_size)
                ...
                if len(offindex) > 0:
                    k = len(offindex)
                    pop_x[offindex, :] = off_x.repeat(k, 1)
                    pop_mean[offindex, :] = off_mean.repeat(k, 1)
                    pop_std[offindex, :] = off_std.repeat(k, 1)
    ...
    def _operator_DE(self, Parent1, Parent2, Parent3):
        ...
        val = 2.0 * mu + (1.0 - 2.0 * mu) * ((1.0 - delta1).pow(disM + 1))
        Offspring[temp] = Offspring[temp] + ((val[temp]).pow(1.0 / (disM + 1)) - 1.0) * U_L[temp]
        # mu > 0.5
        temp = torch.logical_and(Site, mu > 0.5)
        val = 2.0 * (1.0 - mu) + 2.0 * (mu - 0.5) * ((1.0 - delta2).pow(disM + 1))
