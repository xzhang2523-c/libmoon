...
class DirHVEGOSolver(MOBOD):
    ...
    def _step(self, batch_size):
        ...
        for i in range(batch_size):
            index = int(torch.argmax(torch.sum(temp, dim=1)).item())
            Qb.append(index)
            beta = beta + temp[index, :]
            # Update temp: [EI_D(x|\lambda) - beta]_+
            temp = EIDs - beta.unsqueeze(0).repeat(pop_size, 1)
            temp[temp < 0] = 0
    ...
    def _get_xis(self, ref_vecs):
        ...
        Lmin = torch.min(G, dim=1, keepdim=True).values  # N*1  one for each direction vector
