...
from libmoon.util.mtl import model_from_dataset, mtl_dim_dict, mtl_setting_dict, get_dataset, numel
...
from libmoon.solver.gradient import EPOCore, PMGDACore
...
class GradBasePSLMTLSolver:
    def __init__(self, problem_name, batch_size, step_size, epoch, device, solver_name):
        ...
        self.hnet = HyperNet( kernel_size = self.kernel_size ).to(self.device)
        self.net = LeNetTarget( kernel_size = self.kernel_size ).to(self.device)
        ...
        for epoch_idx in tqdm(range(self.epoch)):
            ...
            for batch_idx, batch in enumerate(self.train_loader):
                ...
                if self.is_agg:
                    ...
                    grads_arr = [0,] * len(loss_vec)
                    for loss_idx, loss in enumerate(loss_vec):
                        ...
                        grad_list = dict_grad.values()
                        flat_grad = torch.cat([g.flatten() for g in grad_list])
                        grads_arr[loss_idx] = flat_grad
                    grads_arr = torch.stack(grads_arr)
                    n_obj, n_var = grads_arr.shape
                    if self.solver_name == 'epo':
                        epo_core = EPOCore(n_var, ray.unsqueeze(0))
                        alpha = epo_core.get_alpha(Jacobian=grads_arr, losses = loss_vec, idx=0)
                    else:
                        pmgda_core = PMGDACore(n_var, ray.unsqueeze(0))
                        alpha = pmgda_core.get_alpha(Jacobian=grads_arr, losses = loss_vec, idx=0)
                    loss = alpha @ loss_vec
                else:
    ...
    def eval(self, n_eval):
        uniform_prefs = torch.Tensor(get_prefs(n_eval)).to(self.device)
        loss_pref = []
