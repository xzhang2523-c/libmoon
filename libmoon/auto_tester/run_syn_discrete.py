...
import numpy as np
import sys
from libmoon.util import synthetic_init, get_problem, get_prefs
from libmoon.solver.gradient.methods.base_solver import GradBaseSolver
from libmoon.solver.gradient import (EPOCore, PMGDACore, MGDAUBCore, RandomCore, AggCore,
                                     PMTLCore, GradHVCore, MOOSVGDCore)
import os
...
def save_figures(folder_name):
    ...
    print('Save fig svg to {}'.format(fig_name_svg))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--solver-name', type=str, default='agg_tche')
    parser.add_argument( '--problem-name', type=str, default='VLMOP1')      # (用大写)
    ...
    if args.solver_name == 'epo':
        ...
        agg_name = args.solver_name.replace('agg_', '')
        ...
        core_solver = GradHVCore(n_obj=problem.n_obj, n_var=problem.n_var, problem_name=problem.problem_name)
    elif args.solver_name == 'pmtl':
        core_solver = PMTLCore(n_obj=problem.n_obj, n_var=problem.n_var, n_epoch=args.epoch, prefs=prefs)
    else:
