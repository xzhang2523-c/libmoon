...
from libmoon.util.constant import FONT_SIZE, get_agg_func, root_name


from libmoon.solver.psl.core_psl import BasePSLSolver
import argparse
import os
from libmoon.util import get_prefs
...
if __name__ == '__main__':
    ...
    args.eval_num = 50 if problem.n_obj == 2 else 200
    ...
    prefs = get_prefs(n_prob=args.eval_num, n_obj=problem.n_obj, clip_eps=1e-2)
    if args.device == 'cpu':
        eval_y = problem.evaluate(model( Tensor(prefs))).cpu().detach().numpy()
    else:
